...
| Pricing Calculator | **newest v10 → v11** | Rebuilt on the newest v10 (corrected ROI base). *Example Quotes*: anchored existing MG Red Shift row as **v4.0**; added **v4.1** (template reply) and **v4.2 + v4.1** (dual intake). ROI engine inherited unchanged from newest v10. | Converts clean; ordering v4.1 < v4.0 < v4.2; ROI formula = corrected |
...
## 4a. ROI correction (important)

The **repo v10** (which the first v11 draft was built on) computed First-Year ROI as
`(AnnualNetBenefit − Setup) / Setup` — denominator = **setup only**, ignoring the 12 months of
managed-service cost. This overstates ROI badly (e.g. MGREDSHIFT ≈ **675%**).

The **newest v10** (uploaded) fixes this: `Total Year-1 Investment = Setup + 12×Monthly`, and
First-Year ROI = `(AnnualNetBenefit − Setup) / (Setup + 12×Monthly)` (MGREDSHIFT ≈ **104%**). It
also adds a **Year 2+ Annual ROI** = net benefit ÷ annual managed cost. **v11 is now rebuilt on
this corrected v10**, so the ROI is right.

Note: the newest v10 also contains two unrelated scratch sheets (`Numerologia`, `Numertologia VD`)
and backup ROI tabs (`ROI App_v0`, `ROI App_v1`) — consider removing before sharing externally.
